...
| 岗位案卷（筛选池左列） | 案卷头（小节名「岗位案卷」+ 置灰收藏 / 大标题 / 公司 / 内联指标 `薪资 | 地点 | 经验 | 学历 | 行业 | 融资 | 规模` / 来源·招聘官·更新时间 / 操作行，未接入即置灰）+「岗位概览」双列表 + JD 分节卡（编号圆圈列表 / 单段说明 / 短词标签三种形态按内容自适应；小节名与条数取自 JD 原文）+ 公司档案（公司信息·招聘官信息·公司介绍·工商信息，默认收起） |
| 评分占位（筛选池右列） | 引擎未接入：圆环只显示「○ 待评分」、维度条为空的轨道 + `—`，禁用「重新分析」；评分、建议与关键词都必须有原文或人审依据，不得先写死示例数值 |
...
- 主流程动线与导航级顺序一致（级数以 01 篇页面地图为准）；**候选区是唯一投递入口**，其他页面不出现进入投递包的按钮。
- 每屏一个主操作；右栏永远是当前对象的「第二视角」（证据/Guard/时间线三选一）。
- 筛选池为顶部岗位横条 + 两栏：左「岗位案卷」（案卷头内联人工确认操作：添加到候选区/忽略）、右「AI 建议与评分 → 匹配关键词 → 公司档案四折叠」；该页隐藏 ContextRail（右栏即第二视角）；≤1023 单列堆叠。
...
| 筛选池（顶部横条 + 岗位案卷（案卷头含操作）+ 右栏评分/关键词/公司档案） | Notion §Tables/文档表面 + Vercel §data-table；评分区用 AiDraftCard 的「未确认」语言 |
