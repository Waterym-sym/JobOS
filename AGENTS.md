...
本项目是 **AI Resume OS / JobOS v1.1**：个人本地单机求职闭环（BOSS 采集 → 筛选 → JD/匹配 → 人工候选 → 三形态简历+招呼语 → 人工发送 → 聊天回采 → 复盘）。
...
8. 禁止修改 ADR、本文件、24 篇红线、验收阈值；禁止删除/跳过/弱化测试或 Gold Set；禁止自动调权/自动应用策略。
...
- 服务只绑 `127.0.0.1`；单用户本地系统，无多租户、无云 SaaS 设计。
