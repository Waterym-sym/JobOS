...
> **2026-09-18 目标架构补充（待独立人审）**：本人已批准从下述 v1.1 单用户本机基线转向多用户产品；新决策见 [ADR-013](architecture/decisions/ADR-013-多用户与公网入口.md)，架构评审任务见 [P1-007](delivery/tasks/P1-007-multiuser-architecture-gate.md)。本文原 v1.1 章节保留为历史基线与当前实现说明；涉及“无用户体系／全部入口 loopback”的目标性陈述待 ADR-013 接受后由新版本替代，**当前代码仍不是多用户系统**。

> **分层边界**：目标仅允许经 TLS 与用户认证的 Web 入口对外；API/扩展 WS、数据库、Redis、Worker 及管理接口不得直接公开。本机扩展采集与聊天回采不接入公网账号，聊天原文仍不出本机。自动外发、Truth Guard 与采集风控规则不变。本期只交付代码及本地验收，公网部署需另行 Go/No-Go。
...
新增待人审决策：[ADR-013 多用户身份隔离与公网 Web 入口](architecture/decisions/ADR-013-多用户与公网入口.md)。它只覆盖本版 ADR-005 的单用户/全入口 loopback 适用范围；不改 ADR-006/009 或其它安全红线。具体的身份、owner、旧数据归属、注销与迁移约束以该新 ADR 为目标方案，独立评审通过前不得作为可实施/可发布基线。
